...
$(function(){
	$('.modBtn').on('click',function(){
		...
		$(location).attr('href', '/board/modBoardView?user_id='+user_id+"&board_no="+board_no);
	})
	...
	$('.profileDownBtn').on('click', function(){
		...
		$(location).attr('href', '/board/profileDownload?file_no='+file_no);
	})
...
<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
	<div class="row">
		<div class="col-sm-8 blog-main">
			<div id="header">
				<input id="board_no" type="text" readonly
					value="${BoardVo.board_no }" hidden="hidden"> <input
					id="kind_no" type="text" readonly value="${BoardVo.kind_no }"
					hidden="hidden">
			...
			<div id="cont">
				...
				<c:forEach items="${filelist }" var="FileVo">
					...
					<label>${FileVo.realfilename }</label>
					<br>
					<img
						src="/board/profileImgView?file_no=${FileVo.file_no}"
						style="height: 100px; width: 100px;" />
					<br>
					<button type="button" class="btn btn-default profileDownBtn"
						data-file_no="${FileVo.file_no}">다운로드</button>
					<br>
				</c:forEach>
			...
			<div id="reply">
				...
				<c:forEach items="${replylist }" var="reply">
					...
					<br>
						...
						<input class="modBtn_reply" data-reply_no="${reply.reply_no }"
							data-user_id="${reply.user_id }"
							data-reply_cont="${reply.reply_cont }" type="button" value="수정"
							style="float: right;">

						<input class="delBtn_reply" data-reply_no="${reply.reply_no }"
							data-user_id="${reply.user_id }"
							data-reply_cont="${reply.reply_cont }" type="button" value="삭제"
							style="float: right;">
					</c:if>
					<br>
					<hr>
				...
				<div id="replyRegist">
					<form action="replyregist" method="POST">
						<input name="board_no" type="text" value="${BoardVo.board_no }"
							hidden="hidden">
						<textarea name="reply_cont" rows="5" cols="120"
							placeholder="욕설과 모욕적인 표현은 피해 주세요 :)"></textarea>
